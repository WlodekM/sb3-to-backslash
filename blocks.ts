...
export function jsBlocksToJSON(jsblocks = globalThis.Blockly.Blocks) {
    ...
    const processedBlocks = Object.fromEntries(
        Object.entries(blocks).map(([opcode, block]) => {
            ...
            const args: any[][] = Object.keys(block)
                .filter(a => a.startsWith('args'))
                .map(n => block[n])
                .filter(a => a[0]?.type != 'field_image');
            ...
            if(args.some(k => k.some(a=>a.type == 'input_statement'))) {
                const branchInputs = args.filter(k => (k.find(a => a.type == 'input_statement'))).reduce((p, c) => [...p, ...c], []);
                ...
                }) ?? [], 'branch', branchInputs.map(i => i.name)]]
            }
            return [opcode, [((args[0] ?? []).map((arg: any) => {
                if (arg.type == 'field_dropdown') {
                    return { //TODO - in some way implement this
